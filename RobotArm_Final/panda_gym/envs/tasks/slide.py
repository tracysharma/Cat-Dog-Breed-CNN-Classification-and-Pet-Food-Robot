...
class Slide(Task):
    ...
    def _create_scene(self) -> None:
        ...
        self.sim.create_cylinder(
            ...
            position=np.array([-0.2, 0.23, self.object_size / 2]),
    ...
    def reset(self) -> None:
        ...
        self.sim.set_base_pose("target", self.goal, np.array([0.2, 0.0, 0.0, 1.0]))
        self.sim.set_base_pose("objectCat", self._sample_object(0), np.array([0.0, 0.1, 0.0, 1.0]))
        self.sim.set_base_pose("objectDog", self._sample_object(1), np.array([0.0, -0.1, 0.0, 1.0]))
    ...
    def _sample_object(self, animal) -> np.ndarray:
        ...
        if animal == 0:
            object_position = np.array([-0.2, 0.23, self.object_size / 2])
        else:
            object_position = np.array([-0.2, -0.23, self.object_size / 2])
    ...
    def compute_reward(self, observation, achieved_goal, desired_goal, info: Dict[str, Any]) -> np.ndarray:
        ...
        achieved_goal_copy = achieved_goal.copy()

        # First training reward (Moving behind the can in preparation to push forward)
        # (1500 episodes)
        achieved_goal_copy[0] -= 0.1 # offset to behind the can relative to the goal
        achieved_goal_copy[2] += 0.025 # z-offset to avoid the end effector touching the ground
        d = distance(observationCopy[:3], achieved_goal_copy[:3])

        # Second training reward (Moving the can forward towards the goal)
        # (1500 episodes)
        # achieved_goal_copy[0] -= 0.08 # Almost touching the object
        # achieved_goal_copy[2] += 0.025
        # d = distance(observationCopy[:3], achieved_goal_copy[:3])
        # d = d + 3*distance(achieved_goal, desired_goal) # Heavily reward moving correct can to goal
